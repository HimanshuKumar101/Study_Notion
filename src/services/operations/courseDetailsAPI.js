...
export const addCourseDetails = async (data, token) => {
  ...
  try {
    const response = await apiConnector("POST", CREATE_COURSE_API, data, {
      ...
      Authorization: `Bearer ${token}`, // Fixed typo: "Authorisation" to "Authorization"
    });
...
export const markLectureAsComplete = async (data, token) => {
  ...
  try {
    const response = await apiConnector(
      "POST", 
        LECTURE_COMPLETION_API,
      { courseId: data.courseId, subsectionId: data.subsectionId },
      { Authorization: `Bearer ${token}` }
    );
    ...
    if (error.response?.data?.enrollmentRequired) {
      toast.error("Please enroll in the course first");
      // Consider redirecting to course details page
